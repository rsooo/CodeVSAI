...
//  var field  = Array.ofDim[Cell](100,100)
  ...
  def findUnit(fromy : Int, fromx : Int, num : Int, unitType: UNIT_TYPE) : Iterable[FieldUnit] = {
    ...
    val distance: Iterable[(FieldUnit, Int)] = for(unit <- myUnitMap.values if unit.unitType == unitType && !unit.command.isInstanceOf[EarnResourceCommand]) yield (unit, COMMON.distance(unit.y, unit.x, fromy, fromx))
